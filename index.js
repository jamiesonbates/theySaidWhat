...
// -----------------------------------------------------------------------------
  ...
  const organizeData = function() {
    ...
    const statementsInclusive = dataSet.objects.map((statement) => {
      ...
      if (parties.includes(statement.speaker.party.party) && statement.speaker.first_name !== '' && !allStatements.includes(`http://www.politifact.com${statement.canonical_url}`)) {
        const statementObj = {
    ...
    for (const statement of statementsObjSet) {
      allStatements.push(statement.statementUrl);
    }
    // console.log(statementsObjSet);
...
// -----------------------------------------------------------------------------
  ...
  const partTwo = function($target) {
    ...
    statementsObjSet[setCount].truthGuess = answer;

    if (statementsObjSet[setCount].truthGuess === statementsObjSet[setCount].ruling) {
      $('#ruling').text(`Correct! The statement is ${statementsObjSet[setCount].ruling}`);
    }

    if (statementsObjSet[setCount].truthGuess !== statementsObjSet[setCount].ruling) {
      $('#ruling').text(`Incorrect! The statement is actually ${statementsObjSet[setCount].ruling}`);
    }

    $('#prompt h2').text('');

    $('#truth-photo').removeClass('off');
    $('#truth-photo').attr('src', statementsObjSet[setCount].rulingGraphic);
    $('#summary-header').removeClass('off');
    $('#ruling-summary').text(statementsObjSet[setCount].rulingSummary);
    $('#source').removeClass('off');
    $('#source').attr('href', statementsObjSet[setCount].statementUrl);
  ...
  const nextQuestion = function() {
    ...
    $('#result').text('');
    $('#ruling').text('');
    $('#quote-count').text(`Quote ${quoteCount} of ${allStatements.length}`);
    $('#submit').addClass('off');
